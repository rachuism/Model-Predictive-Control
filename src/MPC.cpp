...
size_t N = 10; //this is empirically
double dt = 0.1;
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (unsigned int i = 0; i < N; i++) {
      fg[0] += 2900*CppAD::pow(vars[epsi_start + i], 2);
      fg[0] += 2900*CppAD::pow(vars[cte_start + i], 2);
      fg[0] += CppAD::pow(vars[v_start + i] - ref_vel, 2);
    }

    for (unsigned int i = 0; i < N - 1; i++) {
       fg[0] += 690*CppAD::pow(vars[delta_start + i] * vars[v_start+i], 2);
    ...
    for (unsigned int i = 0; i < N - 2; i++) {
      fg[0] += 190*CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += 10*CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }

    //For bumping the position 
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //Upper and lower limits of the non actuators
